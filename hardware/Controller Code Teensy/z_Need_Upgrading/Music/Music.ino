...
#define DEVICE_FRIENDLY_NAME "Music Puzzle"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "Music"
...
void loop()
{
  ...
  {
    mythra.publishText("Events", "event", "heartbeat");
...
void checkButton(int pin, bool &currentState, bool &lastState, unsigned long &lastDebounceTime, char buttonName)
{
  ...
  if ((millis() - lastDebounceTime) > debounceDelay)
  {
    if (reading != currentState)
    {
      ...
      if (currentState)
      {
        // Send button number immediately using MythraOS
        char buttonMessage[2];
        buttonMessage[0] = buttonName;
        buttonMessage[1] = '\0';
        mythra.publishText("Events", "event", buttonMessage);
