...
// ------------------- CONSTANTS & VARIABLES -------------------
#define DEVICE_ID "Chemical"
#define DEVICE_FRIENDLY_NAME "Chemical Puzzle"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "Chemical"
...
void setup()
{
  ...
  mythra.setCommandCallback([](const char *command, const JsonDocument &payload, void *context) {
    if (strcmp(command, "actuator") == 0 && payload.is<const char*>()) {
      actuatorMove(payload.as<const char*>());
    }
  });
}
...
void publishAllPresentTags()
{
  ...
  bool dataChanged = (!telemetryInitialized) || (strcmp(combined, lastCombinedTags) != 0);

  if (dataChanged && mythra.isConnected())
  {
    Serial.print("[PUBLISHING] ");
    Serial.println(combined);
    mythra.publishText("Telemetry", "data", combined);
