...
#define DEVICE_ID "Gear"
#define DEVICE_FRIENDLY_NAME "Gear Puzzle"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "Gear"
...
#define POWERLED 13 // or your actual LED pin
...
void loop()
{
  ...
  if (currentMillisMqtt - previousMillisMqtt > MQTTDELAY)
  {
    ...
    char countersData[32];
    snprintf(countersData, sizeof(countersData), "%ld:%ld", counterA, counterB);

    // CHANGE DETECTION: Only publish if counter values changed and MQTT is connected
    bool dataChanged = (!telemetryInitialized) || (strcmp(countersData, lastTelemetry) != 0);

    if (dataChanged && mythra.isConnected())
    {
      Serial.print("[PUBLISHING] Counters: ");
      Serial.print(counterA);
      Serial.print(":");
      Serial.println(counterB);

      mythra.publishText("Telemetry", "counters", countersData);
      strcpy(lastTelemetry, countersData);
