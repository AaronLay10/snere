...
#define DEVICE_ID "Syringe"
#define DEVICE_FRIENDLY_NAME "Syringe Puzzle"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "Syringe"
...
void setup()
{
  ...
  mythra.setCommandCallback([](const char *command, const JsonDocument &payload, void *context) {
    if (strcmp(command, "actuators") == 0 && payload.is<const char*>()) {
      actuators(payload.as<const char*>());
    }
  });
...
void loop()
{
  ...
  if (dataChanged && mythra.isConnected())
  {
    ...
    mythra.publishText("Telemetry", "encoders", telemetry);
