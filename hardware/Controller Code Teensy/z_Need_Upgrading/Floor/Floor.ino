...
// Device ID for MQTT
#define DEVICE_ID "Floor"
#define DEVICE_FRIENDLY_NAME "Floor Puzzle"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "Floor"
...
void drawerState()
{
  ...
  if (!stepperActive)
  {
    ...
    mythra.publishText("Telemetry", "data", "Motor control activated");
  }
  ...
  if (isMoving && checkProximitySensors())
  {
    ...
    mythra.publishText("Telemetry", "data", "Motor stopped by proximity sensor");
  }
}
...
void moveToOpen()
{
  ...
  if (isMoving)
  {
    ...
    mythra.publishText("Telemetry", "data", "Motor already moving");
    return;
  }
  ...
  mythra.publishText("Telemetry", "data", "Moving to OPEN position");
  drawerMoving = true;
...
void moveToClose()
{
  ...
  mythra.publishText("Telemetry", "data", "Moving to CLOSE position");
  drawerMoving = true;
...
void stopMotor()
{
  ...
  mythra.publishText("Telemetry", "data", "Motor stopped manually");
}
...
void testMotorPins()
{
  ...
  mythra.publishText("Telemetry", "data", "Motor pin test completed");
}
...
void drawerControl(const char *value)
{
  ...
  else
  {
    ...
    mythra.publishText("Telemetry", "data", buffer);
  }
}
...
void leverState()
{
  ...
  if (!irReceiverActive)
  {
    ...
    mythra.publishText("Telemetry", "data", "State5:IR_Auto_Active,Photocell_ON");
  }
...
void activateSolenoid()
{
  ...
  solenoidActive = true;
  mythra.publishText("Telemetry", "data", "Solenoid activated");
}
...
void solenoidControl(const char *value)
{
  ...
  else if (cmd == "off")
  {
    digitalWrite(CUCKCOOSOLENOID, LOW);
    solenoidActive = false;
    mythra.publishText("Telemetry", "data", "Solenoid deactivated");
  }
}
...
void activateIR(const char *value)
{
  ...
  if (strcmp(value, "1") == 0 || strcmp(value, "on") == 0)
  {
    ...
    mythra.publishText("Telemetry", "data", "IR_Manual_Override:ON");
  }
  else if (strcmp(value, "0") == 0 || strcmp(value, "off") == 0)
  {
    ...
    mythra.publishText("Telemetry", "data", "IR_Manual_Override:OFF");
  }
}
...
void testFloorButtons()
{
  ...
  if (currentTime - lastButtonCheck >= BUTTON_CHECK_DELAY)
  {
    ...
    mythra.publishText("Telemetry", "data", buttonStatus);
