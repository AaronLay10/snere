...
#define DEVICE_ID "StudyD"
#define DEVICE_FRIENDLY_NAME "Study D Controller"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "StudyD"
...
// Configuration function for MythraOSMQTT
...
// Telemetry change detection - publish only when data changes
...
void setup()
{
    ...
    // Register Command Callback for all commands
    mythra.setCommandCallback([](const char *command, const JsonDocument &payload, void *context) {
        if (payload.is<const char*>()) {
            ...
            if (strcmp(command, "activate") == 0) {
                ...
                stopMotorsHandler(value);
            } else if (strcmp(command, "reset") == 0) {
                resetHandler(value);
            } else if (strcmp(command, "testRight") == 0) {
                testRightHandler(value);
            } else if (strcmp(command, "testLeft") == 0) {
                testLeftHandler(value);
            } else if (strcmp(command, "testPins") == 0) {
                testPinsHandler(value);
            } else if (strcmp(command, "fogIntensity") == 0) {
                fogIntensityHandler(value);
            } else if (strcmp(command, "fogTrigger") == 0) {
                fogTriggerHandler(value);
            } else if (strcmp(command, "fogMachine") == 0) {
                fogMachineHandler(value);
            } else if (strcmp(command, "dmxTest") == 0) {
                dmxTestHandler(value);
            }
        }
    });
...
void loop()
{
    ...
    char telemetry[200];
    sprintf(telemetry, "act:%d,run:%d,L:%d,R:%d,dir:%s,LS:%d%d%d%d,RS:%d%d%d%d",
            systemActive ? 1 : 0,
            motorsRunning ? 1 : 0,
...
void setFogMachine(uint8_t volume, uint8_t timer, uint8_t fanSpeed)
{
    ...
    {
        Serial.println("ANTARI Z-350 DMX UPDATE:");
    ...
    {
        Serial.println("DMX values unchanged - no update needed");
    }
}
