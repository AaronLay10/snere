...
#define DEVICE_ID "Riddle"
#define DEVICE_FRIENDLY_NAME "Riddle Puzzle"
#define ROOM_ID "Clockwork"
#define PUZZLE_ID "Riddle"
#define POWERLED 13
...
void setup()
{
  ...
  // Initialize Ethernet and MQTT with MythraOS
  mythra.begin();
  ...
  mythra.setCommandCallback([](const char *command, const JsonDocument &payload, void *context) {
    ...
    if (strcmp(command, "door") == 0 && payload.is<const char*>()) {
      motorMove(payload.as<const char*>());
    }
    if (strcmp(command, "maglock") == 0 && payload.is<const char*>()) {
      leverMaglock(payload.as<const char*>());
    }
  });
...
void readSensors()
{
  ...
  {
  case STARTUP:
    sprintf(telemetryData, "State:%01X,Ready", state);
    break;

  case KNOBS:
    sprintf(telemetryData, "%01X,%01X:%01X:%01X,%01X:%01X,%01X:%01X:%01X:%01X,%01X:%01X:%01X:%01X,%01X:%01X:%01X:%01X,%01X:%01X:%01X:%01X,%01X:%01X:%01X:%01X,%01X:%01X,%01X",
            state,
  ...
  case MOTORS:
    sprintf(telemetryData, "State:%01X,Direction:%d,Running:%d,Sensors_UP:%01X:%01X,Sensors_DN:%01X:%01X,DoorLoc:%d",
  ...
  case LEVER:
    sprintf(telemetryData, "State:%01X,Photocell:%d,GunCode:0x%lX,MagLock:%01X",
            state, analogRead(PHOTOCELL), receivedGunCode, digitalRead(MAGLOCK));
    break;

  case GUNS:
    sprintf(telemetryData, "State:%01X,GunCode:0x%lX", state, receivedGunCode);
    break;

  case FINISHED:
    sprintf(telemetryData, "State:%01X,Complete", state);
    break;

  default:
    sprintf(telemetryData, "State:%01X,Unknown", state);
    break;
  }
