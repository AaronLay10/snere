...
export function useExecutorSocket(options?: UseExecutorSocketOptions) {
  ...
  const baseUrl = useMemo(() => {
    ...
    if (typeof window !== 'undefined') {
      const envUrl = process.env.NEXT_PUBLIC_EXECUTOR_WS;
      ...
      // Preferred: same-origin reverse proxy path (expects Nginx to proxy /scene-executor → executor service)
      // This avoids hardcoding a port that may not be publicly exposed.
      const sameOriginPath = `${wsProto}://${loc.host}/scene-executor`;

      // Secondary: direct port (useful in local dev)
      const directPort = `${wsProto}://${loc.hostname}:3004`;

      // Try same-origin by default; socket.io will emit 'connect_error' if unreachable and we can fallback.
      // We return the same-origin URL first; optional runtime fallback handled below on connect_error.
      return `${sameOriginPath}||${directPort}`; // encode both and parse on init
    }
    return undefined;
  }, []);
