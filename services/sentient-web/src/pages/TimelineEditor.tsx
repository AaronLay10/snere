    ...
    <div ref={setNodeRef} style={style} className={cardClass}>
      <div className="flex items-center gap-4">
        ...
        <div className="flex-shrink-0">
          <div
            className={`w-12 h-12 rounded-xl bg-${color}-500/20 border-2 border-${color}-500/40 flex items-center justify-center`}
          >
            <span className={`text-lg font-bold text-${color}-400`}>{step.step_number}</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span
              className={`px-2 py-1 text-xs font-medium rounded bg-${color}-500/20 text-${color}-400 uppercase`}
            >
              {step.step_type}
            </span>
            {isCurrent && (
          ...
          {step.description && (
            <p className="text-sm text-gray-400 line-clamp-2 mb-2">{step.description}</p>
          )}
          ...
          {step.timing_config?.delayAfterMs && step.timing_config.delayAfterMs > 0 && (
            <div className="flex items-center gap-4 text-xs text-cyan-400 mt-2">
              <span className="flex items-center gap-1">
                <Clock className="w-3 h-3" />
                Delay After:{' '}
                {step.timing_config.delayAfterMs >= 1000
...
export default function TimelinePage() {
  ...
  const {
    ...
    lastSceneEvent,
    lastAction,
  } = useExecutorSocket({ roomId: roomId || undefined });
  ...
  useEffect(() => {
    ...
    if (lastSceneEvent.scene.id !== sceneId) {
      console.log(
        '[Timeline] Scene event for different scene:',
        lastSceneEvent.scene.id,
        'vs',
        sceneId
      );
      return;
    ...
    try {
      ...
      const audioDevs = (devicesData.devices || []).filter(
        (dev: Device) =>
          ...
          (dev.device_id?.toLowerCase().includes('scs') &&
            dev.device_id?.toLowerCase().includes('audio'))
      );
  ...
  const handleAddStep = async () => {
    ...
    if (newStep.step_type === 'audio') {
      ...
      if (
        ...
        !newStep.config.params?.durationMs
      ) {
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      ...
      (!newStep.execution_interval || newStep.execution_interval < 100)
    ) {
  ...
  const handleTestStep = async (step: SceneStep) => {
    ...
    try {
      ...
      const errorMsg =
        error.response?.data?.message || error.message || 'Failed to test timeline step';
  ...
  const handleSaveToJson = async () => {
    ...
    try {
      ...
      toast(
        `${result.data.sequence_length} actions, ${Math.round(result.data.total_duration_ms / 1000)}s duration`,
        {
          icon: 'ℹ️',
        }
      );
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message ||
          error.message ||
          'Failed to start scene on scene-executor'
      );
    } finally {
      setTestingTimeline(false);
    }
  ...
  const renderStepConfig = () => {
    if (newStep.step_type === 'video') {
      ...
      const selectedVideoDevice = videoDevices.find(
        (d) => d.device_id === newStep.config.device_id
      );
      ...
      return (
        <div className="space-y-4">
          <div>
            ...
            {videoDevices.length === 0 && (
              <p className="text-xs text-yellow-500 mt-1">
                No video playback devices found. Ensure your video player device has
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Optional: Override the default video duration. Leave empty to use the command's
              registered duration.
            </p>
          </div>
        </div>
            ...
            <>
              ...
              {newStep.config.command === 'playCue' && (
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">The SCS cue ID to play</p>
                </div>
              )}
        ...
        <div className="space-y-4">
          <div>
            ...
            {effectDevices.length > 0 && filteredDevices.length === 0 && (
              <p className="text-xs text-gray-500 mt-1">No devices match your search.</p>
            )}
          </div>
          ...
          {newStep.config.device_id && (
            <div>
              ...
              {availableCommands.length > 0 && filteredCommands.length === 0 && (
                <p className="text-xs text-gray-500 mt-1">No commands match your search.</p>
              )}
            </div>
          )}
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      ...
                      {selectedPuzzle.difficulty_rating && (
                        <span className="text-orange-400">
                          Difficulty: {selectedPuzzle.difficulty_rating}/5
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs text-cyan-400">
                <strong>Note:</strong> The puzzle's timeline and win conditions are configured in
                the Puzzle Editor. This step will execute the puzzle's full timeline when the scene
                reaches this step.
              </div>
            </div>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {steps.length > 0 &&
              ['admin', 'game_master', 'technician'].includes(user?.role || '') && (
                <>
                  <button
                        ...
                        {Object.entries(stepTypeIcons).map(([type, Icon]) => {
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                setNewStep({ ...newStep, step_type: type as any, config: {} })
                              }
                            ...
                            >
                              <Icon
                                className={`w-8 h-8 mx-auto mb-2 ${isSelected ? `text-${color}-400` : 'text-gray-500'}`}
                              />
                              <p
                                className={`text-sm font-medium capitalize ${isSelected ? `text-${color}-400` : 'text-gray-400'}`}
                              >
                                {type}
                              </p>
                            </button>
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Must be completed before advancing</p>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Can be triggered multiple times</p>
                      </div>
                    </label>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      ...
                      <span className="ml-2 text-xs text-gray-500">
                        (Wait time after step completes)
                      </span>
                    </label>
                    <input
                      type="number"
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Time to wait after this step executes before moving to the next step. Example:
                      TV needs 3 seconds to power on → enter 3000
                    </p>
                  </div>

                  {/* Execution Mode (not for stopLoop, puzzle, or wait types) */}
                  {newStep.step_type !== 'stopLoop' &&
                    newStep.step_type !== 'puzzle' &&
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                              <Play
                                className={`w-6 h-6 mx-auto mb-2 ${newStep.execution_mode === 'once' ? 'text-cyan-400' : 'text-gray-500'}`}
                              />
                              <p
                                className={`text-sm font-medium ${newStep.execution_mode === 'once' ? 'text-cyan-400' : 'text-gray-400'}`}
                              >
                                Run Once
                              </p>
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                const autoLoopId =
                                  deviceId && action ? `${deviceId}_${action}` : '';
                                setNewStep({
                                  ...newStep,
                                  execution_mode: 'loop',
                            ...
                            >
                              <Repeat
                                className={`w-6 h-6 mx-auto mb-2 ${newStep.execution_mode === 'loop' ? 'text-orange-400' : 'text-gray-500'}`}
                              />
                              <p
                                className={`text-sm font-medium ${newStep.execution_mode === 'loop' ? 'text-orange-400' : 'text-gray-400'}`}
                              >
                                Loop at Interval
                              </p>
                        ...
                        {newStep.execution_mode === 'loop' && (
                          <>
                            <div>
                              ...
                              <p className="text-xs text-gray-500 mt-1">
                                Current:{' '}
                                {newStep.execution_interval && newStep.execution_interval >= 60000
                            ...
                            <div>
                              ...
                              />
                              <p className="text-xs text-gray-500 mt-1">
                                Auto-generated based on device and action. Used to stop this loop
                                later.
                              </p>
                            </div>
                          </>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-sm text-gray-300">
                        ...
                        <ul className="space-y-1 text-xs">
                          <li>
                            <strong>Puzzle:</strong> Interactive challenge requiring player solution
                          </li>
                          <li>
                            <strong>Video:</strong> Visual media playback (narrative, instructions)
                          </li>
                          <li>
                            <strong>Audio:</strong> Voice acting, dialog, sound effects
                          </li>
                          <li>
                            <strong>Effect:</strong> Lighting, fog, mechanical movements
                          </li>
                          <li>
                            <strong>Wait:</strong> Pause for player action or game master
                            intervention
                          </li>
                        </ul>
                      </div>
              ...
              {steps
                ...
                .map((step) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-orange-400">{step.loop_id}</p>
                        <p className="text-xs text-gray-400 mt-1">
                          {step.name} • Every{' '}
                          {step.execution_interval && step.execution_interval >= 60000
