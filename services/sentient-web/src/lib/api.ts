...
// Use relative URL to work with nginx proxy - browser will use same origin
  ...
  (error: AxiosError) => {
    if (error.response?.status === 401) {
      ...
      if (typeof window !== 'undefined') {
        ...
        // Don't redirect if on touchscreen page (public interface) or already on login
        const pathname = window.location.pathname;
        if (pathname !== '/login' && pathname !== '/touchscreen') {
...
// Rooms
export const rooms = {
  getAll: async (params?: { client_id?: string; status?: string; limit?: number; offset?: number }) => {
...
export const devices = {
  getAll: async (params?: { room_id?: string; category?: string; emergency_stop_required?: boolean }) => {
...
// Scenes
export const scenes = {
  getAll: async (params?: { room_id?: string }) => {
  ...
  executeScene: async (id: string, skip_safety: boolean = false) => {
    const response = await apiClient.post(`/api/sentient/scenes/${id}/execute`, { skip_safety });
    return response.data;
  },

  // Start scene on executor (proxy through API to avoid CORS)
  startScene: async (id: string, options?: { skip_safety?: boolean }) => {
    const response = await apiClient.post(`/api/sentient/scenes/${id}/start`, options || {});
    return response.data;
  },

  // Reset scene on executor (proxy through API to avoid CORS)
  resetScene: async (id: string) => {
    const response = await apiClient.post(`/api/sentient/scenes/${id}/reset`, {});
    return response.data;
  },
...
export const puzzles = {
  ...
  // Start puzzle on executor (proxy through API to avoid CORS)
  startPuzzle: async (id: string, options?: { skip_safety?: boolean }) => {
    const response = await apiClient.post(`/api/sentient/puzzles/${id}/start`, options || {});
    return response.data;
  },

  // Reset puzzle on executor (proxy through API to avoid CORS)
  resetPuzzle: async (id: string) => {
    const response = await apiClient.post(`/api/sentient/puzzles/${id}/reset`, {});
    return response.data;
  },
};
...
// Users
export const users = {
  getAll: async (params?: { client_id?: string; role?: string; active?: boolean }) => {
  ...
  uploadPhoto: async (id: string, file: File) => {
    ...
    const response = await apiClient.post(`/api/sentient/users/${id}/photo`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
  ...
  deletePhoto: async (id: string) => {
    const response = await apiClient.delete(`/api/sentient/users/${id}/photo`);
    return response.data;
  },
};
...
export const sceneSteps = {
  getAll: async (params?: { scene_id?: string }) => {
  ...
  reorder: async (scene_id: string, steps: { id: string; step_number: number }[]) => {
...
export const controllers = {
  getAll: async (params?: { room_id?: string; status?: string; hardware_type?: string }) => {
  ...
  executeAction: async (id: string, action_id: string, parameters?: Record<string, any>) => {
...
export const mqtt = {
  ...
  // Touchscreen unauthenticated publish (limited to lighting topics)
  touchscreenPublish: async (topic: string, payload: any) => {
    const response = await apiClient.post('/api/mqtt/touchscreen/publish', {
      topic,
      payload,
    });
    return response.data;
