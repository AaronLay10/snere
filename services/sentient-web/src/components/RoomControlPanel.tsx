...
export default function RoomControlPanel({ roomSlug = 'Clockwork' }: RoomControlPanelProps) {
  ...
  const handleLightControl = async (lightId: string, action: 'on' | 'off') => {
    ...
    try {
      // Map light IDs to command names
      const commandMap: Record<string, string> = {
        'boiler': 'boilerLights',
        'study': 'studyLights',
        'lab': 'labLights',
        'crawlspace': 'crawlspaceLights'
      };

      const commandName = commandMap[lightId];
      if (!commandName) {
        throw new Error(`Unknown light: ${lightId}`);
      }

      // New topic format: paragon/clockwork/commands/main_lighting/{command}
      const topic = `paragon/clockwork/commands/main_lighting/${commandName}_${action}`;
      await mqtt.publish(topic, { params: {}, timestamp: Date.now() });
