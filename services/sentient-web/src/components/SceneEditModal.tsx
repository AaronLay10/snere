/**
 * Scene Edit Modal Component
 * Modal for editing existing scene properties
 */
...
export default function SceneEditModal({
  ...
  const [formData, setFormData] = useState<SceneFormData>({
    ...
    slug: scene.slug
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '_')
      .replace(/^_+|_+$/g, ''), // Sanitize existing slug
    sceneNumber: scene.scene_number || '',
    sceneType: scene.scene_type || scene.type || 'scene',
    description: scene.description || '',
    estimatedDuration: scene.estimated_duration_seconds ? Math.round(scene.estimated_duration_seconds / 60) : '',
    transitionType: scene.transition_type || 'manual',
    autoAdvance: scene.auto_advance || false,
    autoAdvanceDelay: scene.auto_advance_delay_ms || ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the scene
      await scenes.update(scene.id, sceneData);

      toast.success(`Scene "${formData.name}" updated successfully`);
      onSceneUpdated(); // Let parent handle closing
    } catch (err: any) {
      console.error('Update scene error:', err);
      const errorMessage = err.response?.data?.message || 'Failed to update scene';
      ...
      <div className="bg-gray-900 border border-cyan-500/30 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gray-900 border-b border-cyan-500/30 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-cyan-400">Edit Scene</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                disabled={saving}
              >
                <option value="scene">Scene (Timeline-based)</option>
                <option value="puzzle">Puzzle (Logic-based)</option>
                <option value="cutscene">Cutscene</option>
                <option value="transition">Transition</option>
              </select>
              <p className="text-xs text-gray-500 mt-1">
                Select 'Puzzle' to configure this scene in the Puzzles editor
              </p>
            </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-700">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-700 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center justify-center gap-2"
            >
