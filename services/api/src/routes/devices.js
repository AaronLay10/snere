...
const deviceSchema = Joi.object({
  ...
  device_category: Joi.string().valid(
    'mechanical_movement', 'sensor', 'switchable_on_off',
    'variable_control', 'media_playback'
  ).required(),
...
router.get('/:id', authenticate, requireCapability('read'), async (req, res) => {
  try {
    ...
    const commandsFromTable = Array.isArray(device.command_names) ? device.command_names.filter(Boolean) : [];
    ...
    if (!capabilities.commands || !Array.isArray(capabilities.commands) || capabilities.commands.length === 0) {
...
router.post('/', authenticate, requireCapability('manage_devices'), async (req, res) => {
  try {
    ...
    const { room_id, device_id, friendly_name, device_category, emergency_stop_required, mqtt_topic, config } = value;
    ...
    const result = await db.query(
      ...
      RETURNING *`,
      [id, room_id, device_id, friendly_name, device_category, emergency_stop_required, mqtt_topic, JSON.stringify(config)]
    );
