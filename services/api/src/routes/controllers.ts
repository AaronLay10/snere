...
import { authenticateServiceOrUser } from '../middleware/serviceAuth.js';
const router = Router();
...
const controllerSchema = Joi.object({
  ...
  capability_manifest: Joi.object({
    ...
    mqtt_topics_publish: Joi.array()
      ...
      .optional(),
    mqtt_topics_subscribe: Joi.array()
      .items(
        Joi.object({
          topic: Joi.string().required(),
...
router.post(
  ...
  requireCapability('manage_devices'),
  async (req, res) => {
    try {
      ...
      const roomLookup = await db.query(`SELECT id FROM rooms WHERE id::text = $1 OR slug = $1`, [
        room_id,
      ]);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updateSchema = controllerSchema.fork(['roomId', 'controllerId'], (schema) =>
        schema.optional()
      );
  ...
  try {
    ...
    if (parseInt(devicesCheck.rows[0].count) > 0) {
      return res.status(409).json({
        success: false,
        message:
          'Cannot delete controller with assigned devices. Please reassign or delete devices first.',
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.query(`SELECT capability_manifest FROM controllers WHERE id = $1`, [
        id,
      ]);
...
router.post(
  '/:id/execute-action',
  authenticateServiceOrUser,
  requireCapability('control_room'),
  async (req, res) => {
    try {
      const { id } = req.params;
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        deviceTypeLower.includes('motor') ||
        deviceTypeLower.includes('servo') ||
        deviceTypeLower.includes('stepper')
      ) {
        device_category = 'mechanical_movement';
      } else if (
        deviceTypeLower.includes('relay') ||
        deviceTypeLower.includes('switch') ||
        deviceTypeLower.includes('valve')
      ) {
        device_category = 'switchable_on_off';
      } else if (
        deviceTypeLower.includes('pwm') ||
        deviceTypeLower.includes('dimmer') ||
        deviceTypeLower.includes('led')
      ) {
