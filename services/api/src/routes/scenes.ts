...
const sceneSchema = Joi.object({
  ...
  transition_type: Joi.string()
    .valid('manual', 'automatic', 'trigger', 'puzzle_complete')
    .optional(),
...
router.put('/:id', authenticate, requireCapability('manage_scenes'), async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && scene.client_id !== req.user.client_id) {
      console.error(
        `Scene update access denied: user.clientId=${req.user.client_id}, scene.client_id=${scene.client_id}, user.role=${req.user.role}`
      );
      return res.status(403).json({
        error: 'Access denied',
        message: 'You do not have access to this scene',
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Get scene details
      const sceneResult = await db.query('SELECT * FROM scenes WHERE id = $1', [sceneId]);
      ...
      // Get room details
      const roomResult = await db.query('SELECT * FROM rooms WHERE id = $1', [scene.room_id]);
      ...
      for (const step of steps) {
        ...
        if (step.step_type === 'puzzle') {
          ...
          await fs.writeFile(puzzleFilePath, JSON.stringify(puzzleJson, null, 2));
      ...
      await fs.writeFile(sceneFilePath, JSON.stringify(sceneJson, null, 2));
