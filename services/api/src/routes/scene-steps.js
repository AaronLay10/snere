...
const sceneStepSchema = Joi.object({
  ...
  step_type: Joi.string().valid('puzzle', 'video', 'audio', 'effect', 'wait', 'stopLoop').required(),
...
router.put('/:id', authenticate, requireCapability('manage_scenes'), async (req, res) => {
  try {
    ...
    const { error, value } = updateSchema.validate(req.body, { allowUnknown: true, stripUnknown: true });
...
router.delete('/:id', authenticate, requireCapability('manage_scenes'), async (req, res) => {
  try {
    ...
    const result = await db.query(
      'DELETE FROM scene_steps WHERE id = $1 RETURNING id, name',
      [id]
    );
...
router.post('/:id/test', authenticate, requireCapability('control_room'), async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        success: true,
        message: executorResponse.data.data?.message || `Test triggered for ${step.step_type} step: ${step.name}`,
        step: step,
