...
const { requireCapability, requireClientAccess } = require('../middleware/rbac');
...
const roomSchema = Joi.object({
  ...
  slug: Joi.string().min(1).max(100).pattern(/^[a-z0-9_-]+$/).required(),
...
router.get('/', authenticate, requireCapability('read'), async (req, res) => {
  try {
    ...
    let query = 'SELECT r.*, c.name as client_name FROM rooms r INNER JOIN clients c ON r.client_id = c.id';
    let params = [];
    let paramIndex = 1;
    let conditions = [];
...
router.get('/:id', authenticate, requireCapability('read'), async (req, res) => {
  try {
    ...
    const scenesResult = await db.query(
      'SELECT COUNT(*) as count FROM scenes WHERE room_id = $1',
      [id]
    );
...
router.post('/', authenticate, requireCapability('write'), async (req, res) => {
  try {
    ...
    const { client_id, name, slug, description, mqtt_topic_base, capacity, duration, difficulty, status, config } = value;
    ...
    const existing = await db.query(
      'SELECT id FROM rooms WHERE client_id = $1 AND slug = $2',
      [client_id, slug]
    );
    ...
    const clientResult = await db.query(
      'SELECT mqtt_namespace FROM clients WHERE id = $1',
      [client_id]
    );
    ...
    const result = await db.query(
      ...
      RETURNING *`,
      [id, client_id, name, slug, description, mqttTopic, capacity, duration, difficulty, status || 'draft', JSON.stringify(config || {})]
    );
...
router.put('/:id', authenticate, requireCapability('write'), async (req, res) => {
  try {
    ...
    const existing = await db.query(
      'SELECT client_id FROM rooms WHERE id = $1',
      [id]
    );
    ...
    const updateSchema = roomSchema.fork(
      ['client_id', 'name', 'slug'],
    ...
    Object.entries(value).forEach(([key, val]) => {
      if (val !== undefined && key !== 'client_id') { // Don't allow changing client
