...
import { requireInterface } from '../middleware/rbac.js';
const router = Router();
  ...
  try {
    ...
    await db.query(
      ...
      [
        ...
        req.user.id,
        'manual_game_master',
        reason || 'Emergency stop triggered by game master',
        true,
      ]
    );
...
router.post('/hint', async (req, res) => {
  try {
    ...
    // Verify room access
    const roomResult = await db.query('SELECT id FROM rooms WHERE id = $1 AND client_id = $2', [
      roomId,
      req.user.client_id,
    ]);
