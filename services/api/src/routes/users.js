...
const { requireRole, requireCapability, canManageRole } = require('../middleware/rbac');
...
const userSchema = Joi.object({
  ...
  role: Joi.string().valid('admin', 'editor', 'viewer', 'game_master', 'creative_director', 'technician').required(),
...
router.delete('/:id/photo', authenticate, async (req, res) => {
  try {
    ...
    const existing = await db.query(
      'SELECT id, profile_photo_url FROM users WHERE id = $1',
      [id]
    );
