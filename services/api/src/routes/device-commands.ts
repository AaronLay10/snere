    ...
    try {
      ...
      if (device.emergency_stop_required && !confirm_safety) {
        return res.status(400).json({
          success: false,
          message:
            'This device requires emergency stop confirmation. Please confirm safety procedures.',
      ...
      const mqttNamespace =
        device.mqtt_namespace ||
        deviceConfig.mqtt_namespace ||
        device.client_namespace ||
        'paragon';
      const mqttRoomId =
        device.mqtt_room_id || deviceConfig.mqtt_room_id || device.room_name || 'Unknown';
      ...
      const payload = {
        value:
          parameters.value || (action === 'turn_on' ? 'on' : action === 'turn_off' ? 'off' : 'on'),
        ...parameters,
      };
